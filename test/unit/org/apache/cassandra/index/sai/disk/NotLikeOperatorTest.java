...
/*
CURRENT STATE OF NOT LIKE OPERATOR:
...
public class NotLikeOperatorTest extends SAITester
{
    @Test
    public void testStringNotLikePrefix() throws Throwable
    {
        ...
        Assert.assertEquals(2, execute("SELECT * FROM " + KEYSPACE + '.' + currentTable() + " WHERE v NOT LIKE 'ab%%'").size());
    }

    @Test
    public void testStringNotLikeSuffix() throws Throwable
    {
        createTable("CREATE TABLE %s (pk int primary key, v text);");
        createIndex("CREATE CUSTOM INDEX ON %s (v) USING 'StorageAttachedIndex'");
        execute("INSERT INTO %s (pk, v) VALUES (?, ?)", 0, "a");
        ...
        execute("INSERT INTO %s (pk, v) VALUES (?, ?)", 2, "ac");
        execute("INSERT INTO %s (pk, v) VALUES (?, ?)", 3, "abcd");
        Assert.assertEquals(3, execute("SELECT * FROM " + KEYSPACE + '.' + currentTable() + " WHERE v NOT LIKE '%%cd'").size());
    }

    @Test
    public void testStringNotLikeContains() throws Throwable
    {
        createTable("CREATE TABLE %s (pk int primary key, v text);");
        createIndex("CREATE CUSTOM INDEX ON %s (v) USING 'StorageAttachedIndex'");
        execute("INSERT INTO %s (pk, v) VALUES (?, ?)", 0, "a");
        ...
        execute("INSERT INTO %s (pk, v) VALUES (?, ?)", 2, "ac");
        execute("INSERT INTO %s (pk, v) VALUES (?, ?)", 3, "abcd");
        Assert.assertEquals(2, execute("SELECT * FROM " + KEYSPACE + '.' + currentTable() + " WHERE v NOT LIKE '%%bc%%'").size());
    }

    @Test
    public void testStringNotLikeMatches() throws Throwable
    {
        createTable("CREATE TABLE %s (pk int primary key, v text);");
        createIndex("CREATE CUSTOM INDEX ON %s (v) USING 'StorageAttachedIndex'");
        execute("INSERT INTO %s (pk, v) VALUES (?, ?)", 0, "a");
        ...
        execute("INSERT INTO %s (pk, v) VALUES (?, ?)", 2, "ac");
        execute("INSERT INTO %s (pk, v) VALUES (?, ?)", 3, "abcd");
        Assert.assertEquals(3, execute("SELECT * FROM " + KEYSPACE + '.' + currentTable() + " WHERE v NOT LIKE 'abc'").size());
    }
}
